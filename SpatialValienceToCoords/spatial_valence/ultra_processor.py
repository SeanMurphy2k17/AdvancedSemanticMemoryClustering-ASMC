...
try:
    ...
    print("         Then run: python -m nltk.downloader sentiwordnet wordnet")
...
class UltraSemanticExtractor:
    ...
    def _analyze_sentiment(self, text: str, words: List[str]) -> Dict[str, float]:
        """Advanced sentiment analysis using SentiWordNet"""
        sentiment = {
            'polarity': 0.0,
            'subjectivity': 0.0,
            'intensity': 0.0,
        ...
        if not SENTIWORDNET_AVAILABLE:
            # Fallback to manual word lists if NLTK not available
            return self._fallback_sentiment_analysis(text, words)
        
        # Use SentiWordNet for comprehensive coverage
        total_pos = 0.0
        total_neg = 0.0
        word_count = 0
        ...
        for i, word in enumerate(words):
            ...
            if i > 0:
                prev_word = words[i-1]
                if prev_word in ['very', 'extremely', 'really', 'quite', 'absolutely']:
                    intensifier = 1.5
                elif prev_word in ['somewhat', 'slightly', 'a bit', 'kinda']:
                    intensifier = 0.7
                elif prev_word in ['not', 'no', 'never', 'hardly']:
                    intensifier = -1.0
            ...
            try:
                ...
                if synsets:
                    ...
                    total_pos += pos_score * intensifier
                    total_neg += neg_score * abs(intensifier)
                    word_count += 1
            except:
                pass  # Word not in SentiWordNet, skip it
        
        # Calculate polarity
        if word_count > 0:
            avg_pos = total_pos / word_count
            avg_neg = total_neg / word_count
            
            sentiment['polarity'] = avg_pos - avg_neg  # Range: -1.0 to +1.0
            sentiment['subjectivity'] = (avg_pos + avg_neg)  # How subjective vs objective
            sentiment['intensity'] = abs(sentiment['polarity'])
            sentiment['confidence'] = min(word_count / len(words), 1.0)
        
        return sentiment
    
    def _fallback_sentiment_analysis(self, text: str, words: List[str]) -> Dict[str, float]:
        """Fallback sentiment analysis if SentiWordNet unavailable"""
        sentiment = {'polarity': 0.0, 'subjectivity': 0.0, 'intensity': 0.0, 'confidence': 0.0}
        ...
        if total > 0:
            ...
            sentiment['confidence'] = min(total / len(words), 1.0)
...
class UltraRobustCoordinateGenerator:
    ...
    def __init__(self, dimensions: int = 9):
        ...
        self.dimension_definitions = {
            ...
            'y': {  # Emotional dimension - PURE sentiment polarity
                'features': ['sentiment.polarity'],
                'weights': [1.0],
                'transform': 'linear'  # Direct mapping: -1 to +1
            },
    ...
    def _get_nested_feature(self, features: Dict, path: str) -> Any:
        """Get nested feature value - handles both flat and nested dicts"""
        # Try direct key lookup FIRST (for flattened dicts like 'sentiment.polarity')
        if path in features:
            return features[path]
        
        # Then try nested traversal (for nested dicts like {'sentiment': {'polarity': 1.0}})
        parts = path.split('.')
        value = features
